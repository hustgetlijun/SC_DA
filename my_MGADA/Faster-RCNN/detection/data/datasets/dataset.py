        ...
        if self.is_sample:
            ...
            if rl_num_boxes > 1:
                ...
                for idx_b in range(len(bboxes_s)):
                    bb = bboxes_s[idx_b]
                    cv2.rectangle(imgv, (int(bb[0]), int(bb[1])), (int(bb[2]), int(bb[3])),
                                  (0, 255 - idx_b * 10, 0 + 10 * idx_b), 3)
...
class VOCDataset(ABSDataset):
    ...
    def __init__(self, root, split='train', base_dir='.', transforms=(), keep_difficult=True, img_ext='.jpg',
                 dataset_name=''):
        self.root = root
        self.split = split
    ...
    def get_annotations_by_image_id(self, img_id):
        ...
        for obj in target['object']:
            ...
            box = obj['bndbox']
            if float(box['xmax']) <= float(box['xmin']) or float(box['ymax']) <= float(box['ymin']):
                print('box is:',box)
                continue
       ...
       # if len(boxes) ==0:
       #     print('there is no object!!!!!', boxes)
       #     return False
        boxes = np.array(boxes).reshape((-1, 4))
        labels = np.array(labels)
