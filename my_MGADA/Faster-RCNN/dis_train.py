...
def train_one_epoch(model, optimizer, train_loader, target_loader, device, epoch, dis_model, dis_optimizer, print_freq= 50, writer=None, test_func=None, save_func=None):
    global global_step
    model.train()
    ...
    for images, img_metas, targets in metric_logger.log_every(train_loader, print_freq, header):
        ...
        pix_class_loss = loss_dict.pop('class_loss')

        #print(adv_loss)
        loss_dict_for_log = dict(**loss_dict, **adv_loss,**pix_class_loss)

        det_loss = sum(list(loss_dict.values()))
        ada_loss = sum(list(adv_loss.values()))

        pix_loss = sum(list(pix_class_loss.values()))

        LAMBDA = cosine_scheduler(cfg.ADV.LAMBDA_FROM, cfg.ADV.LAMBDA_TO, global_step)  #1
        #print(LAMBDA)
        losses = det_loss + pix_loss+ ada_loss * LAMBDA
        optimizer.zero_grad()
        losses.backward()
        optimizer.step()
...
def main(cfg, args):
    ...
    schedulers = [
        torch.optim.lr_scheduler.MultiStepLR(optimizer, cfg.SOLVER.STEPS, gamma=cfg.SOLVER.GAMMA), #表示按需求有外部设置调整学习率
