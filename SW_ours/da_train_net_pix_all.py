...
from model.da_faster_rcnn_allover.resnet import resnet
from model.da_faster_rcnn_allover.vgg16 import vgg16
...
if __name__ == "__main__":
    ...
    for epoch in range(args.start_epoch, args.max_epochs + 1):
        ...
        for step in range(iters_per_epoch):
            ...
            loss = (
                1*category_loss_cls.mean()
                + rpn_loss_cls.mean()
                + rpn_loss_box.mean()
                + RCNN_loss_cls.mean()
                + RCNN_loss_bbox.mean()
                + object_loss.mean()
                + 1*mid_class_loss.mean()
            )
            loss_temp += loss.item()
            ...
            out_d_pixel,out_d_pixel_mid, out_d = fasterRCNN(
                im_data,
                im_info,
                im_cls_lb,
                gt_boxes,
            ...
            if step % args.disp_interval == 0:
                ...
                mid_class_loss = 1*mid_class_loss.mean().item()
                ...
                print(
                    "\t\t\tloss_category_cls: %.4f,class_mid_loss:%.4f, object_loss:%.4f,rpn_cls: %.4f, rpn_box: %.4f, rcnn_cls: %.4f, rcnn_box %.4f dloss s: %.4f dloss t: %.4f dloss s pixel: %.4f dloss t pixel: %.4f,loss_t_mid_d: %.4f loss_s_mid_d: %.4f eta: %.4f, source ins loss: %.4f, target ins loss: %.4f"
                    % (
                        loss_category_cls,
                        mid_class_loss,
                        object_loss,
                        ...
                        loss_s_mid_d,
                        args.eta,
                        # source_ins_da_loss =0,
                        # target_ins_da_loss =0,
                        0,
                        0,
                    )
                )
