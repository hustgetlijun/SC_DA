...
from model.da_faster_rcnn_allover_improve.resnet import resnet
from model.da_faster_rcnn_allover_improve.vgg16 import vgg16
...
def parse_args():
  ...
  """
    ...
    parser.add_argument(
        "--gamma", dest="gamma", help="value of gamma", default=5.0, type=float
    )  #5
    parser.add_argument(
        "--max_epochs",
        dest="max_epochs",
...
if __name__ == "__main__":
    ...
    if args.dataset == "pascal_voc":
        ...
        print("loading our dataset...........")
        args.s_imdb_name = "cityscape_2007_train_s"
        args.t_imdb_name = "BDD100K_2007_train"
        args.s_imdbtest_name = "cityscape_2007_test_s"
        args.t_imdbtest_name = "BDD100K_2007_val"
        args.set_cfgs = [
            "ANCHOR_SCALES",
            "[8,16,32]",
            "ANCHOR_RATIOS",
    ...
    elif args.dataset == "carton":
        print("loading our dataset...........")
        args.s_imdb_name = "Carton_2007_train_CPLC"
        args.t_imdb_name = "Carton_2007_train_ECLC"
        args.s_imdbtest_name = "Carton_2007_test_CPLC"
        args.t_imdbtest_name = "Carton_2007_test_ECLC"
        args.set_cfgs = [
            "ANCHOR_SCALES",
            "[8,16,32]",
            "ANCHOR_RATIOS",
        ...
        args.t_imdb_name = "cityscapes_car_2007_train_s"
        args.s_imdbtest_name = "sim10k_2019_val"
        args.t_imdbtest_name = "cityscapes_car_2007_test_s"
        args.set_cfgs = [
            "ANCHOR_SCALES",
            "[8,16,32]",
            "ANCHOR_RATIOS",
    ...
    for epoch in range(args.start_epoch, args.max_epochs + 1):
        ...
        for step in range(iters_per_epoch):
            ...
            loss = (
                category_loss_cls  #* args.eta
                + rpn_loss_cls.mean()
                + rpn_loss_box.mean()
                + RCNN_loss_cls.mean()
                + RCNN_loss_bbox.mean()
                + object_loss.mean() #* args.eta
                + mid_class_loss.mean() #* args.eta
                # + 0.5*category_consistency_loss_S
            )
            # print('cate + con is :', category_loss_cls.mean()+category_consistency_loss.mean())
            loss_temp += loss.item()
            ...
            # dloss_s = 0.5 * torch.mean(out_d ** 2)

            dloss_s_p = 0.5 * torch.mean((out_d_pixel ** 2)*(1+pix_weight))
            loss_s_mid_d=0.5*torch.mean((out_d_pixel_mid ** 2)*(2-pix_mid_weight))

            # dloss_s_p = 0.5 * torch.mean((out_d_pixel ** 2))
            # loss_s_mid_d=0.5*torch.mean((out_d_pixel_mid ** 2))
            ...
            # im_cls_lb.data.resize_(data_t[2].size()).copy_(data_t[2])
            # gt_boxes.data.resize_(data_t[3].size()).copy_(data_t[3])
            # num_boxes.data.resize_(data_t[4].size()).copy_(data_t[4])

            out_d_pixel,out_d_pixel_mid, out_d,target_ins_da,pix_weight,pix_mid_weight,category_consistency_loss_T= fasterRCNN(
                im_data,
                im_info,
                im_cls_lb,
                gt_boxes,
            ...
            ####################################
            # loss += 0.5*(c1+c2+c3)
            category_consistency_loss = category_consistency_loss_S
            # category_consistency_loss = 0.5*(category_consistency_loss_S+category_consistency_loss_T)
            loss += category_consistency_loss
            ####################################
            ...
            # local alignment loss
            # dloss_t = 0.5 * torch.mean((1 - out_d) ** 2)

            dloss_t_p = 0.5 * torch.mean(((1 - out_d_pixel) ** 2)*(1+pix_weight))
            loss_t_mid_d = 0.5 * torch.mean(((1 - out_d_pixel_mid) ** 2)*(2-pix_mid_weight))

            # dloss_t_p = 0.5 * torch.mean(((1 - out_d_pixel) ** 2))
            # loss_t_mid_d = 0.5 * torch.mean(((1 - out_d_pixel_mid) ** 2))


            if args.dataset == "sim10k":
                loss += (dloss_s + dloss_t + dloss_s_p + dloss_t_p+loss_t_mid_d+loss_s_mid_d) * args.eta
            else:
                loss += (dloss_s + dloss_t + dloss_s_p + dloss_t_p+loss_t_mid_d+loss_s_mid_d)    #*2.0
            ...
            if step % args.disp_interval == 0:
                ...
                loss_category_cls = category_loss_cls.mean().item()
                category_consistency_loss = category_consistency_loss.mean().item()
                ...
                loss_t_mid_d=  loss_t_mid_d.item()
                loss_s_mid_d = loss_s_mid_d.item()
                object_loss = object_loss.mean().item()
                mid_class_loss = mid_class_loss.mean().item()
                ...
                print(
                    "\t\t\tloss_category_cls: %.4f,category_consistency_loss: %.4f,class_mid_loss:%.4f, object_loss:%.4f,rpn_cls: %.4f, rpn_box: %.4f, rcnn_cls: %.4f, rcnn_box %.4f dloss s: %.4f dloss t: %.4f dloss s pixel: %.4f dloss t pixel: %.4f,loss_t_mid_d: %.4f loss_s_mid_d: %.4f eta: %.4f source_instance_Loss: %.4f target_instance_Loss: %.4f"
                    % (
                        loss_category_cls,
