...
from model.da_faster_rcnn_struct.faster_rcnn import _fasterRCNN
...
class netD_pixel(nn.Module):
    def __init__(self, n_class,context=False):
        ...
        self.conv1 = conv1x1(256+n_class, 256)
...
class netD_pixel_mid(nn.Module):
    def __init__(self,n_class,context=False):
        ...
        self.conv1 = conv1x1(512+n_class, 512)
        self.conv1_1 = conv1x1(512, 256)
        # self.bn1 = nn.BatchNorm2d(256)
...
class netD(nn.Module):
    def __init__(self, n_class,context=False):
        ...
        self.conv1 = conv3x3(512+n_class, 512, stride=2)
...
class object_pre(nn.Module):
    def __init__(self,dout_base_model):
        ...
        self.conv1 = nn.Conv2d(self.dout, self.dout, 3, 1, 1, bias=True)
        self.conv2 = nn.Conv2d(self.dout, self.dout, 3, 1, 1, bias=True)
        self.conv3 = nn.Conv2d(self.dout, 1, 1, 1, 0)
    def forward(self, x):
        x = F.relu(x)
        x = F.relu(self.conv1(x))
        x = F.relu(self.conv2(x))
...
class class_pre(nn.Module):
    def __init__(self,dout_base_model,n_class):
        ...
        self.conv1 = nn.Conv2d(self.dout, self.dout, 3, 1, 1, bias=True)
        self.conv2 = nn.Conv2d(self.dout, self.dout, 3, 1, 1, bias=True)
        self.conv3 = nn.Conv2d(self.dout, n_class, 1, 1, 0)
    def forward(self, x):
        x = F.relu(x)
        x = F.relu(self.conv1(x))
        x = F.relu(self.conv2(x))
...
class vgg16(_fasterRCNN):
    ...
    def _init_modules(self):
        ...
        # print(vgg.features)
        self.RCNN_base1 = nn.Sequential(*list(vgg.features._modules.values())[:16])

        self.RCNN_base_mid = nn.Sequential(*list(vgg.features._modules.values())[16:23])
        ...
        self.netD_pixel = netD_pixel(1, context=self.lc)
        self.netD_pix_mid = netD_pixel_mid(self.n_classes-1,context=self.gc)
        self.netD = netD(self.n_classes - 1, context=self.gc)


        self.object_pre = object_pre(256)
        self.classs_pre = class_pre(512,self.n_classes)
