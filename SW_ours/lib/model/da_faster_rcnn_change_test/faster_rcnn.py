...
from model.da_faster_rcnn_change_test.DA import _InstanceDA
...
def save_to_json(data,file_name):
    with open('./save_my_s/{}'.format(file_name),'w') as f:
...
class _fasterRCNN(nn.Module):
    ...
    def forward(
        ...
        pix_data={'data':[], 'label':0}
        pix_data_cat = {'data':[], 'label':0}
        mid_data={'data':[], 'label':0}
        mid_data_cat={'data':[], 'label':0}
        g_data={'data':[], 'label':0}

        # feed image data to base model to obtain base feature map
        base_feat1 = self.RCNN_base1(im_data)
        if self.img_id < 0 :
          pix_data['data']=base_feat1[0].tolist()
          save_to_json(pix_data,'pix_{}.json'.format(self.img_id))
        ...
        #if self.img_id < 0:
        #  pix_data_cat['data']= base_object_feat[0].tolist()
        #  save_to_json(pix_data_cat,'pix_cat_{}.json'.format(self.img_id))


        #base_object_feat = base_feat1

        windows = get_receptive_field_feature(base_feat1, im_data, (32, 32), 4)
        ...
        if self.img_id < 50 :
          mid_data['data']= base_feat_mid[0].tolist()
          save_to_json(mid_data,'mid_{}.json'.format(self.img_id))
        ...
        #if self.img_id < 50:
        #  mid_data_cat['data']= base_class_feat[0].tolist()
        #  save_to_json(mid_data_cat,'mid_cat_{}.json'.format(self.img_id))


        #base_class_feat = base_feat_mid

        # print('feat base shape is :',feat_base.size())
        windows = get_receptive_field_feature(base_feat_mid,im_data,(76,76),8)
        ...
        if self.img_id < 0 :
          g_data['data']= base_feat[0].tolist()
          save_to_json(g_data,'g_{}.json'.format(self.img_id))
        ...
        # base_feat = middle_attention(base_feat, domain_p.detach())
        _, weight_self = self_entropy(domain_p.detach(), True)
