...
def save_to_json(data,file_name):
    with open('./save_ICR_CCR_s_14/{}'.format(file_name),'w') as f:
...
class _fasterRCNN(nn.Module):
    ...
    def forward(
        ...
        base_mid_feat = self.RCNN_base_mid(base_feat1)

        if self.img_id < 50:
          mid_data['data']= base_mid_feat[0].tolist()
          save_to_json(mid_data,'mix_{}.json'.format(self.img_id))

        
        base_feat = self.RCNN_base2(base_mid_feat)
