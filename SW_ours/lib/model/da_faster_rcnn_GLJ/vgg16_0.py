...
from model.da_faster_rcnn_GLJ.faster_rcnn_GLJ import _fasterRCNN
...
class netD_pixel(nn.Module):
    def __init__(self, num_class,context=False):
...
class netD(nn.Module):
    def __init__(self, num_class,context=False):
        ...
        self.conv1 = conv1x1(512+num_class, 512+num_class)
        # self.bn1 = nn.BatchNorm2d(256)
        self.conv2 = conv1x1(512+num_class, 128)
        # self.bn2 = nn.BatchNorm2d(128)
        self.conv3 = conv1x1(128, 1)
...
#     def __init__(self, num_class,context=False):
#         super(netD, self).__init__()
#         self.conv1 = conv3x3(512+num_class, 512, stride=2)
...
class vgg16(_fasterRCNN):
    ...
    def _init_modules(self):
        ...
        self.netD = netD(self.n_classes,context=self.gc)
        self.netD_pixel = netD_pixel(self.n_classes,context=self.lc)
        feat_d = 4096
        if self.lc:
            feat_d += 128
        if self.gc:
