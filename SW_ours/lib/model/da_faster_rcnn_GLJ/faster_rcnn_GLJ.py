...
from model.da_faster_rcnn_GLJ.DA import _InstanceDA
...
from .Semantic_Ca  import Semantic_patch_Ca
...
class _fasterRCNN(nn.Module):
    ...
    def __init__(self, classes, class_agnostic, lc, gc, da_use_contex, in_channel=4096):
        ...
        self.Semantic_patch = Semantic_patch_Ca(self.classes)
    ...
    def forward(
        ...
        pix_label = pix_label.data

        # feed image data to base model to obtain base feature map
        base_feat1 = self.RCNN_base1(im_data)
        # word_vector_local = self.Semantic_patch.Semantic_Ca(im_data, base_feat1, gt_boxes, num_boxes).cuda()
        # _,_,h1,w1 = base_feat1.size()
        # self.adppool1 = nn.AdaptiveAvgPool2d((h1,w1))
        # word_vector_local= self.adppool1(pix_label)

        # if self.lc:
        #     d_pixel, _ = self.netD_pixel(grad_reverse(torch.cat((base_feat1,word_vector_local),1), lambd=eta))
        #     # print(d_pixel)
        #     # if not target:
        #     if True:
        #         _, feat_pixel = self.netD_pixel(torch.cat((base_feat1,word_vector_local),1).detach())
        # else:
        #     d_pixel = self.netD_pixel(grad_reverse(torch.cat((base_feat1,word_vector_local),1), lambd=eta))
        if self.lc:
        ...
        self.adppool1 = nn.AdaptiveAvgPool2d((h2,w2))
        word_vector_global= self.adppool1(pix_label)
        # word_vector_global = self.Semantic_patch.Semantic_Ca(im_data,base_feat, gt_boxes,num_boxes).cuda()

        rois, rpn_loss_cls, rpn_loss_bbox, word_vector_pre = self.RCNN_rpn(
            base_feat, im_info, gt_boxes, num_boxes
        )

        if self.gc:
            domain_p, _ = self.netD(grad_reverse(torch.cat((base_feat,word_vector_pre),1), lambd=eta))
            # if target:
            #     return d_pixel,domain_p#, diff
            _, feat = self.netD(torch.cat((base_feat,word_vector_pre),1).detach())
        else:
            domain_p = self.netD(grad_reverse(torch.cat((base_feat,word_vector_pre),1), lambd=eta))
            # if target:
            #     return d_pixel,domain_p#,diff
        ...
        category_loss_cls = nn.BCEWithLogitsLoss()(word_vector_pre, word_vector_global)
        #################################################################
    ...
    def _init_weights(self):
        ...
        normal_init(self.RCNN_rpn.pix_class_pre, 0, 0.01, cfg.TRAIN.TRUNCATED)
