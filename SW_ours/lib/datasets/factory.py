...
from datasets.Carton import Carton
...
# Set up sim10k coco style and cityscapes coco style
for year in ["2007"]:
    for split in ["train_s", "test_s"]:
        name = "cityscapes_car_{}_{}".format(year, split)
        __sets[name] = lambda split=split, year=year: cityscape_car(split, year)
for year in ["2007"]:
    for split in ["train", "val"]:
        name = "BDD100K_{}_{}".format(year, split)
        __sets[name] = lambda split=split, year=year: BDD100K(split, year)

for year in ["2007"]:
    for split in ["train_CPLC","train_ECLC","train_FM","train_LFM", "train_G_ECLC","train_G_FM",
                  "train_G_LFM","train_G_ECLC_CPLC","train_G_FM_CPLC","train_G_LFM_CPLC",
                  "test_CPLC","test_ECLC","test_FM","test_LFM"]:
        name = "Carton_{}_{}".format(year, split)
        __sets[name] = lambda split=split, year=year: Carton(split, year)
