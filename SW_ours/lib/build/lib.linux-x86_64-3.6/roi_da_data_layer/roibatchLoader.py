...
def get_pix_class_label(image_size,num_class,gt,num_gt):
    W,H = image_size
    pix_label = torch.zeros(num_class,W,H).cuda()
    background = torch.ones(W,H).cuda()
    pix_label[0,:,:] = background.data
    for gt_index in range(num_gt):
        index = int(gt[gt_index][4].data)
        xmin,ymin,xmax,ymax = gt[gt_index][:4].data
        w,h = int(xmax)-int(xmin), int(ymax)-int(ymin)
        class_label = torch.ones(int(w),int(h))
        background_class = torch.zeros(int(w),int(h))
        pix_label[0,int(xmin):int(xmax),int(ymin):int(ymax)] = background_class.data
        pix_label[index, int(xmin):int(xmax), int(ymin):int(ymax)] = class_label.data
    return pix_label
...
class roibatchLoader(data.Dataset):
    ...
    def __getitem__(self, index):  # only one sample
        ...
        """
            ...
            # _,h1,w1 = padding_data.size()
            # pix_label = get_pix_class_label((w1,h1),self._num_classes,gt_boxes_padding,num_boxes)
            # pix_label = pix_label.permute(0,2,1).contiguous()
            ...
            # pix_label = torch.FloatTensor(self._num_classes,h2,w2).zero_()
